...
description: >-
  ...
  timestamps by default, saving the transcript and a copy of the source audio side by side
  under a shared timestamped filename. Use whenever the user says "transcribe this
  recording," "convert this audio to text," "speech to text this file," or points to /
  attaches an audio file and wants a written transcript. Defaults to ~/Recordings as both
  source and output location, named from the recording's own timestamp. Always show a
  step-by-step progress checklist so the user can see which step is running.
compatibility: >-
...
Convert a spoken audio recording into a text transcript, and keep the transcript and the
source audio filed together under one timestamped name.
...
- [ ] Step 5: Save transcript + copy audio
...
- Filename stem = the **source audio file's own modification timestamp**, formatted
  `YYYYMMDD_HHMMSS` — not "now." This keeps the name tied to when the recording actually
  happened, even if transcription runs later.
- Transcript → `<stem>.txt`; copied audio → `<stem><original-extension>`.
...
```bash
~/.mario-skills/recording-transcriber/venv/bin/python scripts/transcribe.py <input_audio> <output.txt> [--verbose] [--no-timestamps]
```
...
- Write the transcript to `<output_folder>/<stem>.txt` (the script already writes directly
  to this path).
- Copy — not move, unless the user explicitly asked to move the source — the source audio
  to `<output_folder>/<stem><original-extension>`.
- If source and destination already coincide (same folder, same name), skip the copy.
