...
2. **If new: what should it be called, and what description should it have?** Offer the auto-generated default title (`Produced by <producers>` for the producer flow, `Albums: <album names>` for the album flow) but let the user override it. Same for the description — offer the default (`Generated by plumbeer-music-producer-playlist.`) but let the user supply their own.
