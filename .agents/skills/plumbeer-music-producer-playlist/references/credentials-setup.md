...
This opens a real browser tab for the user to approve access, then caches the resulting refresh token (default `~/.cache/plumbeer-music-producer-playlist/youtube_token.json`). Every later command reuses it silently — no more browser steps, and expired access tokens are refreshed automatically.
...
test -f ~/.cache/plumbeer-music-producer-playlist/youtube_token.json && echo "Cached token found (auth already done)" || echo "No cached token — run 'auth' first"
```
