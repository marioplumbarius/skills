...
user what to do next, per plumbeer-music-producer-playlist/SKILL.md.
...
DEFAULT_TOKEN_CACHE = os.path.expanduser("~/.cache/plumbeer-music-producer-playlist/youtube_token.json")
...
def cmd_execute(args):
    ...
    if args.mode == "create":
        ...
        if plan.get("producers"):
            ...
            default_description_summary = (
                f"Tracks produced by {producer_names} — {track_count} songs from their production "
                "catalogs, including work for other artists."
            )
        else:
            album_names = ", ".join(a["album"] for a in plan.get("albums", []) if "tracks" in a)
            artist_names = ", ".join(dict.fromkeys(a["artist"] for a in plan.get("albums", []) if "tracks" in a))
            default_title = f"Albums: {album_names}"
            default_description_summary = f"All tracks from {artist_names}'s albums {album_names} — {track_count} songs."
        title_suffix = " (built by plumbeer)"
        base_title = args.title or default_title
        title = base_title[: 150 - len(title_suffix)] + title_suffix
        ...
        base_description = args.description or default_description_summary
        description = base_description[: 5000 - len(description_footer)] + description_footer
        try:
            resp = requests.post(
...
def main():
    ...
    p_execute.add_argument(
        ...
        default=None,
        help=(
            "Only used with --mode create. Sets the first line of the description "
            "(the built-by-plumbeer footer is always appended). Defaults to an "
            "auto-generated music summary."
        ),
    )
